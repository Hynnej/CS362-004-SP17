...
int main(){
	...
	for (i = 0; i < TEST_NUMBER; i++) {
		...
		if(gameInit == 0){
			cardPlay = great_hallPlay(player, &stateTest, handPo);
			if(cardPlay == 0){
...
int initializeTestGame(int k[10], int player, struct gameState *game){
	...
	game->hand[player][game->handCount[player]] = great_hall;
...
int testCard(int player, struct gameState *game, struct gameState *gameTest){
	...
	if ((gameTest->numActions + 2) == game->numActions){
		printf("Success - number of actions.\n");
	}else{
